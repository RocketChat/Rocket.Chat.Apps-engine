...
export class HttpAccessorTestFixture {
	...
	public async useHttp() {
		...
		Expect(this.mockPreRequestHandler.executePreHttpRequest).toHaveBeenCalledWith(
			'url-here',
			request3,
			this.mockReader,
			this.mockPersis,
		...
		Expect(await http.post('url-here')).toBeDefined();
		Expect(this.mockPreResponseHandler.executePreHttpResponse).toHaveBeenCalledWith(
			this.mockResponse,
			this.mockReader,
			this.mockPersis,
		);

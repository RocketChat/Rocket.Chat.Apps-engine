...
export class AppSettingsManagerTestFixture {
	...
	public async updatingSettingViaAppSettingsManager() {
		...
		await Expect(async () => await asm.updateAppSetting('fake', TestData.getSetting())).toThrowErrorAsync(
			Error,
			'No App found by the provided id.',
		);
		await Expect(async () => await asm.updateAppSetting('testing', TestData.getSetting('fake'))).toThrowErrorAsync(
			Error,

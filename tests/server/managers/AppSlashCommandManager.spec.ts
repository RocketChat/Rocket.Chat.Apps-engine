...
export class AppSlashCommandManagerTestFixture {
	...
	public addCommand() {
		...
		Expect((ascm as any).appsTouchedCommands.get('testing').length).toBe(1);
		Expect(() => ascm.addCommand('another-app', cmd)).toThrowError(
			CommandHasAlreadyBeenTouchedError,
			'The command "my-cmd" has already been touched by another App.',
		);
		Expect(() => ascm.addCommand('testing', cmd)).toThrowError(
			CommandAlreadyExistsError,
			'The command "my-cmd" already exists in the system.',
		);
		Expect(() => ascm.addCommand('failMePlease', TestData.getSlashCommand('yet-another'))).toThrowError(
			Error,
		...
		Expect(() => ascm.addCommand('even-another-app', TestData.getSlashCommand('it-exists'))).toThrowError(
			CommandAlreadyExistsError,
	...
	public failToModifyAnotherAppsCommand() {
		...
		Expect(() => ascm.modifyCommand('testing', TestData.getSlashCommand('my-cmd'))).toThrowError(
			CommandHasAlreadyBeenTouchedError,
			'The command "my-cmd" has already been touched by another App.',
		);
	}
	...
	public failToModifyNonExistantAppCommand() {
		...
		Expect(() => ascm.modifyCommand('failMePlease', TestData.getSlashCommand('yet-another'))).toThrowError(
			Error,
	...
	public modifyMyCommand() {
		...
		Expect(() => ascm.modifyCommand('testing', TestData.getSlashCommand())).toThrowError(
			Error,
	...
	public enableMyCommand() {
		...
		Expect(() => ascm.enableCommand('testing', 'doesnt-exist')).toThrowError(
			Error,
			'The command "doesnt-exist" does not exist to enable.',
		);
	...
	public failToEnableAnotherAppsCommand() {
		...
		Expect(() => ascm.enableCommand('my-app', 'command')).toThrowError(
			CommandHasAlreadyBeenTouchedError,
	...
	public disableMyCommand() {
		...
		Expect(() => ascm.disableCommand('testing', 'doesnt-exist')).toThrowError(
			Error,
			'The command "doesnt-exist" does not exist to disable.',
		);
		ascm.addCommand('testing', TestData.getSlashCommand('command'));
	...
	public failToDisableAnotherAppsCommand() {
		...
		Expect(() => ascm.disableCommand('my-app', 'command')).toThrowError(
			CommandHasAlreadyBeenTouchedError,
			'The command "command" has already been touched by another App.',
		);
	}
	...
	public registerCommands() {
		...
		Expect((ascm as any).registerCommand as FunctionSpy)
			...
			.exactly(1);
		Expect(this.mockBridges.getCommandBridge().doRegisterCommand)
			.toHaveBeenCalledWith(enabledRegInfo.slashCommand, 'testing')
			.exactly(1);
	}
